...
export class AuthentificationService {
   users : AppUser[]=[]; //tableau users
  authenticatedUser !: AppUser;
  constructor() {
    this.users.push({userId:uuid.v4(), username:"user1",password: "1234",roles: ["USER"]});
    this.users.push({userId:uuid.v4(), username:"user2",password: "1234",roles: ["USER"]});
    this.users.push({userId:uuid.v4(), username:"admin",password: "1234",roles: ["USER","ADMIN"]});
  }
